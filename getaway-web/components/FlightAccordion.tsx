...
import { PlaneTakeoff, CircleMinus, BookmarkCheck, PlaneLanding, RefreshCw } from 'lucide-react';
...
export function FlightAccordion({ flights, loading, group = false, mockdata = false }: FlightAccordionProps) {
    ...
    return (
      <div className="flex flex-col justify-center items-center py-20 text-center space-y-4 rounded-3xl border border-white/40" 
        ...
        <div className="space-y-2">
          ...
          <p style={{ color: "#6b6560" }} className="max-w-xs mx-auto">Try adjusting your search or retrying.</p>
        </div>
        <div className="flex gap-4">
          <Button variant="outline" className="rounded-full bg-white/50" onClick={() => window.location.reload()}>Reset Filters</Button>
  ...
  return (
    <div className="w-full rounded-3xl p-7 pt-3 animate-fade-up-4"
      style={{
      ...
      <div className="space-y-1">
        {flights.length > 0 && (
          <div className="flex flex-row items-center justify-center w-full mb-4 gap-8">
            ...
            <Button onClick={() => window.location.reload()}
              className="group flex justify-center items-center gap-2 px-6 rounded-2xl transition-all duration-300 hover:shadow-md hover:scale-105 active:scale-95"
        ...
        <Accordion type="multiple" className="w-full space-y-4" defaultValue={processedData?.map(group => group.label)}>
          {processedData?.map((group) => (
            <AccordionItem key={group.label} value={group.label} className="border-none">
              <AccordionTrigger className="hover:no-underline p-0 opacity-80 hover:opacity-100 justify-center">
                <div className="relative flex py-2 items-center w-full">
                  <div className="flex-grow border-t-2" style={{ borderColor: "rgba(252, 173, 108, 0.6)" }}></div>
                  <Badge variant="outline" className="border-6 mx-4" style={{ background: "rgba(253, 196, 130, 0.66)", borderColor: "rgba(252, 173, 108, 0.6)" }}>
                    <span className="text-sm font-md uppercase tracking-widest">{group.label} ({group.flights.length})</span>
                  </Badge>
                  <div className="flex-grow border-t-2" style={{ borderColor: "rgba(252, 173, 108, 0.6)" }}></div>
                </div>
              </AccordionTrigger>
              <AccordionContent className="pt-2 space-y-4">
                <Accordion type="single" collapsible className="w-full space-y-3">
                  {group.flights.map((flight, idx) => (
                    <AccordionItem key={`${group.label}-flight-${idx}`} value={`item-${idx}`}
                      className="border-none rounded-2xl overflow-hidden transition-all duration-300 hover:shadow-xl relative"
                      style={{ background: "rgba(255, 255, 255, 0.75)", backdropFilter: "blur(25px)", border: "1px solid rgba(255, 255, 255, 0.8)" }}>
                      {!!flight.savedEntry && (
                        <div className="absolute top-0 left-3 z-30 pointer-events-none">
                          <div className="bg-[#c4714a] text-white px-1 pb-1 pt-2 rounded-b-md shadow-md flex items-center justify-center">
                      ...
                      <AccordionTrigger className="hover:no-underline p-4">
                        <div className="flex flex-row justify-between items-center w-full">
                          <div className="flex flex-col items-center gap-1 border border-gray-200/30 rounded-xl p-3 w-36 shadow-sm divide-y-2 divide-grey-700" style={{ background: "rgba(255, 243, 233, 0.8)" }}>
                            <span className="font-bold text-lg" style={{ color: "#1a1714" }}>{flight.legs[0]?.airline || "NA"}</span>
                            <span className="text-xs uppercase tracking-wider opacity-60">{flight.cabin}</span>
                          </div>
                          ...
                          <div className="text-right border border-gray-200/30 rounded-xl p-3 w-32 shadow-sm" style={{ background: "rgba(255, 243, 233, 0.8)" }}>
                            <span className="text-xl font-bold" style={{ color: "#c4714a" }}>${parseFloat(flight.price).toFixed(2)}</span>
                            <p className="text-[10px] uppercase font-bold opacity-40">{flight.currency}</p>
                          </div>
                        </div>
                      </AccordionTrigger>
                      <AccordionContent className="px-6 pb-4 pt-2" style={{ background: "rgba(253, 252, 249, 0.75)" }}>
                        <FlightTimelineContent flight={flight} saveFlights={saveFlights} removeFlights={removeFlights} />
                      </AccordionContent>
                    </AccordionItem>
                  ))}
                </Accordion>
...
function FlightTimelineContent({ flight, saveFlights, removeFlights }: { flight: ProcessedFlight; saveFlights: (flight: Flight) => void; removeFlights: (id: string) => void; }) {
  const sharedBg = { background: "rgba(255, 243, 233, 0.8)", backdropFilter: "blur(20px)" };
  return (
    <div className="space-y-2">
      <div className="border rounded-xl border-gray-200 overflow-x-auto shadow-sm" style={sharedBg}>
      ...
      <div className="flex items-center justify-center pt-2">
        {flight.booking_url && (
          <a href={flight.booking_url} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()}>
            <Button className="w-48 rounded-full shadow-md transition-all hover:scale-105" style={{ background: "#c4714a", color: "white" }}>Book This Trip</Button>
          </a>
        )}
        <div className="pl-4">
          <SaveButton saved={!!flight.savedEntry} onSave={() => saveFlights({ ...flight })} onRemove={() => flight.savedEntry && removeFlights(flight.savedEntry.id)} />
        </div>
      </div>
    </div>
  );
...
function LegNode({ seg, landing }: { seg: Segment, landing?: boolean }) {
  ...
  if (!dep || !arr) return <div className="px-4 py-1.5 rounded-xl bg-white/30">Unable to obtain flight data</div>;

  return (
    <div className="flex items-center gap-12 px-4 py-1.5 rounded-xl bg-white/30">
      <FlightNode airport={legs[0].origin} time={dep.departure_time} compact={false} />
      <div className="flex flex-col items-center gap-1 min-w-[140px]">
        <div className="flex items-center justify-center p-1 rounded-xl border border-gray-200/50 backdrop-blur-sm" style={{ background: "rgba(250, 223, 201, 0.8)" }}>
          {landing ? <PlaneLanding className="w-6 h-6 -scale-x-100 pb-1" style={{ color: "#b65325" }} /> : <PlaneTakeoff className="w-6 h-6 pb-1" style={{ color: "#c4714a" }} />}
        </div>
        <Badge variant="secondary" className="rounded-sm bg-white/50 border-gray-200/50 px-1 font-medium" style={{ background: "rgba(235, 162, 102, 0.9)" }}>{formatDuration(seg.duration)}</Badge>
        {seg?.total_stops > 0 && (
          <Badge variant="outline" className="rounded-sm py-0 px-1 border-gray-300 text-xs" style={{ background: "rgba(197, 138, 90, 0.1)" }}>
            {seg.total_stops} <CircleMinus className="w-3 h-3 ml-1" style={{ color: "#c4714a" }} />
          </Badge>
        )}
      </div>
      <FlightNode airport={arr.destination} time={arr.arrival_time} compact={false} />
    </div>
  );
}

function FlightNode({ airport, time, compact }: { airport: string; time: string; compact: boolean }) {
  const city = airportsjs.lookupByIataCode(airport).city;
  return (
    <div className={`flex flex-col text-center items-center justify-center gap-0.5 ${compact ? "w-20" : "w-40"}`}>
      <div className="flex flex-wrap justify-center items-baseline gap-1.5">
        <span className="text-lg" style={{ color: "#1a1714" }}>{city}</span>
        <span className="font-mono opacity-70 uppercase tracking-tighter">{airport}</span>
      </div>
      <Badge variant="outline" className="rounded-sm bg-gray-300/10 border-gray-300 px-1 font-medium">{formatTime(time)}</Badge>
    </div>
  );
}